...
```bash
cargo run -p cli -- deploy chat examples/chat-worker/dist/worker.js --memory-binding CHAT_ROOM --public
```
...
```bash
just fly-proxy
just fly-worker-deploy chat examples/chat-worker/dist/worker.js --memory-binding CHAT_ROOM --public
```
