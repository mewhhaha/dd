...
impl BlobStore {
    pub(crate) async fn for_legacy_root(root: PathBuf) -> Result<Self> {
        fs::create_dir_all(&root).await.map_err(blob_error)?;
        Ok(Self { root })
    }

    pub(crate) async fn read_legacy(&self, blob_ref: &str) -> Result<Vec<u8>> {
    ...
    pub(crate) async fn delete_legacy(&self, blob_ref: &str) -> Result<()> {
        let id = parse_local_ref(blob_ref)?;
    ...
    #[cfg(test)]
    pub(crate) async fn put(&self, bytes: &[u8]) -> Result<String> {
...
#[cfg(test)]
pub(crate) async fn local_blob_store_for_tests(root: impl AsRef<Path>) -> Result<BlobStore> {
