...
fn print_help() {
    ...
    println!("Core env:");
    println!(
        "  DD_BENCH_ONLY                  comma-separated sections: steady-state, websocket, lifecycle, kv-writes"
    );
    println!(
...
export async function onSocketMessage(stub, event) {
  await stub.atomic((tx) => {
    const payload = typeof event.data === "string" ? `echo:${event.data}` : event.data;
...
export default {
  async fetch(request, env) {
    ...
    return await memory.atomic((tx) => openSocket(tx, { request }));
  },
...
async fn start_service(tag: &str, runtime: RuntimeConfig) -> common::Result<RuntimeService> {
    ...
    start_service_with_paths(runtime, &paths.store_dir).await
}
