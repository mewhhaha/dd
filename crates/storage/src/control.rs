...
impl ControlStore {
    ...
    pub async fn deactivate_deployment(&self, worker: &str, deployment_id: &str) -> Result<bool> {
        let worker = worker.to_string();
        ...
        let changed = self
            .with_write_transaction(move |tx| {
                ...
                Box::pin(async move {
                    tx.execute(
                        "DELETE FROM active_deployments WHERE worker_name = ?1 AND deployment_id = ?2",
...
mod tests {
    ...
    #[tokio::test]
    async fn expiring_an_old_deployment_preserves_its_replacement() -> Result<()> {
        let root = temp_dir("expiry-replacement");
        let store = ControlStore::open(&root).await?;
        store.insert_deployment(&deployment("worker", 1)).await?;
        store.insert_deployment(&deployment("worker", 2)).await?;
        assert!(
            !store
                .deactivate_deployment("worker", "deployment-1")
                .await?
        );
        let active = store.active_deployments().await?;
        assert_eq!(active.len(), 1);
        assert_eq!(active[0].deployment_id, "deployment-2");
        assert!(
            store
                .deactivate_deployment("worker", "deployment-2")
                .await?
        );
    ...
    #[tokio::test]
    async fn rejects_future_control_schema() -> Result<()> {
